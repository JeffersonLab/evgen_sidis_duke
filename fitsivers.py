...
# at ~5.7 s per replica that is ~48 min for a Sivers fit, against ~19 min at 200).
...
# needs a range.
#
# Measured, not assumed: on datasivers_phifull a 500-replica pool gives an
# across-ensemble scatter of 0.06-0.12 at Nrep = 50 for the f1Tperp bands against
# the 0.101 predicted above. Unlike the Collins fit, the Sivers replicas are
# unimodal (0 of 500 in any secondary minimum), so nothing inflates the formula
# here; the noisiest parameters are au and cu at ~2x it. Evidence: check.md.
...
# Replica knobs as flags: ./fitsivers.py <opt> <rundir> [-n NREP] [-s SEED0].
...
# environment any more, and a leftover `NREP=10 ./fitsivers.py ...` -- the form every
...
PARS = ('Nu','au','bu','cu','Nd','ad','bd','cd','Nub','Ndb','kt2')
...
def fitfunc0(var):
    ...
    res = []
    x, Q2, z, pT, target, hadron, value, error = worldrep['x'].values, worldrep['Q2'].values, worldrep['z'].values, worldrep['pT'].values, worldrep['target'].values, worldrep['hadron'].values, worldrep['value'].values, worldrep['error'].values
    for i in range(len(worldrep)):
        res.append( (tmd.AUTSivers(x[i],Q2[i],z[i],pT[i],target[i],hadron[i],par) - value[i])**2 / error[i]**2 )
    return np.sum(res)
...
def fitfunc(var):
    ...
    for i in range(len(worldrep)):
        res.append( (tmd.AUTSivers(x[i],Q2[i],z[i],pT[i],target[i],hadron[i],par) - value[i])**2 / error[i]**2 )
    x, Q2, z, pT, target, hadron, value, error = simdatarep['x'].values, simdatarep['Q2'].values, simdatarep['z'].values, simdatarep['pT'].values, simdatarep['target'].values, simdatarep['hadron'].values, simdatarep['value'].values, simdatarep['error'].values
...
# model at these same start values (chi2 ~ 1e-11), so migrad barely moves, and the
# recomputed simdata['value'] reproduces what prepare.py wrote from the identical
# par1. The machinery is general; it is a no-op only because the world dataset is
# synthetic. Mirrors simulate() in fitcollins.py -- keep the two in step.
def simulate(data):
    ...
    Min.strategy=1
    Min.migrad()
    par0 = Min.values        # name-keyed ValueView -- tmd.AUTSivers needs par['Nu'] etc.
    var0 = Min.np_values()   # positional ndarray -- from_array_func(start=...) needs a sequence
                             # both include fixed params (cu, cd, Nub, Ndb), so downstream
                             # sees all 11. NB par0 is a live view on Min, not a copy; safe
                             # here because it is consumed below, before Min goes out of scope.
    val = []
...
if __name__ == "__main__":
    print(f'Running the fit to Sivers function ... ({rundir})', end = '\n')
    if opt == 'world':
        ...
        print('fitting SBS+CLAS12+SoLID enhanced (including syst) ...', end='\n')
