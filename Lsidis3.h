...
class Lsidis{
 ...
 public:
  ...
  int GenerateEventKinematics(const int method);//Sample and calculate an event's kinematics without evaluating PDFs/FFs; follow with GetWeightFromCurrentState to get the weight
  double GetWeightFromCurrentState(const int mode);//Evaluate PDFs/FFs and the cross section weight for the kinematics set by GenerateEventKinematics
...
int Lsidis::CalculateFinalState(){//Calculate scattered electron and detected hadron from x, y, z, Pt, phih, phiS
  int ret = CalculateFinalStateKinematics();
  if (physics_control){
    GetPDFs();
    GetFFs();
  }
...
int Lsidis::CalculateFinalStateKinematics(){//Same as CalculateFinalState but without evaluating PDFs/FFs
...
int Lsidis::GenerateEventKinematics(const int method = 0){//Sample and calculate an event's kinematics without evaluating PDFs/FFs; follow with GetWeightFromCurrentState to get the weight
  double var[6];
  for (int i = 0; i < 6; i++){
    var[i] = gRandom->Uniform(Xmin[i], Xmax[i]);
  }
  ...
  CalculateFinalStateKinematics();
  return physics_control ? 1 : 0;
}

double Lsidis::GetWeightFromCurrentState(const int mode = 0){//Evaluate PDFs/FFs and the cross section weight for the kinematics set by GenerateEventKinematics
  if (!physics_control) return 0.0;
  GetPDFs();
  GetFFs();
  double weight = dsigma(mode);
  return weight * volume * jacobian;
}
